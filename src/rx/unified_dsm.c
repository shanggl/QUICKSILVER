...
dsm_do_more:
  ...
  case DSM_CHECK_PROTOCOL: {
    if (!serial_read_bytes(&serial_rx, &rx_data[1], 1)) {
      return PACKET_NEEDS_MORE;
    }
    const dsm_protocol_t proto = detect_protocol(rx_data[1]);
    if (proto == DSM_PROTO_INVALID) {
      // maybe they are fades??
      rx_data[0] = rx_data[1];
      goto dsm_do_more;
    }
    if (dsm_protocol == DSM_PROTO_INVALID) {
      dsm_protocol = proto;
    }
